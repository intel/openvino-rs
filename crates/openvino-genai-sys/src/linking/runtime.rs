    ...
    (
        ...
        pub fn load() -> Result<(), String> {
            ...
            match $crate::library::find() {
                None => Err("Unable to find the `openvino_genai_c` library to load".into()),
        ...
        /// the `openvino_genai_c` shared library file (e.g., `libopenvino_genai_c.so`).
        ///
        /// If the library has already been loaded (via [`load`] or a prior `load_from`
        /// call), this is a no-op — whichever call succeeds first wins.
        ///
        /// # Errors
        ///
            ...
            pub unsafe fn $name($($pname: $pty), *) $(-> $ret)* {
                let f = with_library(|l| {
                    l.functions.$name.expect(concat!(
                        "`openvino_genai_c` function not loaded: `",
                        stringify!($name)
                    ))
                }).expect("an `openvino_genai_c` shared library is not loaded on this thread");
                f($($pname), *)
            }
        )+
    )
